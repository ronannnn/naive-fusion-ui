import type { Column, HeaderOperationsProps, StorageColumn, StorageTableSetting, TableOrderQueryOption, TableProps, WhereQueryOption, WhereQueryProps } from '@/index'
import type { OrderQuery, StatsItem, WhereQuery, WhereQueryItem } from '@/shared'
import type { HTMLAttributes, Ref, VNode } from 'vue'
import { FDropdownButton, FPopconfirmButton, FTooltipButton } from '@/index'
...
export function useTable<T>(props: TableProps<T>) {
  ...
  const whereQueryInitValues = computed<WhereQuery<T>>(() => {
    const initValues: WhereQueryItem<T>[] = whereQueryOptions.value
      .filter(option => option.initValues)
      .map(option => ({ field: option.field, opr: option.opr ?? null, value: option.initValues ?? null } as WhereQueryItem<T>))
    extraWhereQueryInitValues?.forEach((initValue) => {
      initValue.items?.forEach(item => initValues.push(item)) // TODO:
    })
    return initValues
  })
  const orderQueryOptions = computed<TableOrderQueryOption<T>[]>(() => {
    // options from columns
    const options: TableOrderQueryOption<T>[] = columns
  ...
  const whereQuery = computed<WhereQuery<T>>({
    get: () => {
        ...
        render: (rowData) => {
          ...
          rowActions?.forEach((action) => {
            ...
            if (action.type === 'tooltip') {
              ...
              actions.push(
                h(FDropdownButton, {
                  options: action.dropdownButtonOptions(rowData),
                  size: 'small',
                  type: 'primary',
                  tertiary: true,
                  circle: true,
            ...
            summary[item.field] = {
              value: h('div', { class: 'flex-y-center gap-1' }, [
                !isEmptyString(item.rmk) && h(NTooltip, { disabled: isEmptyString(item.rmk) }, {
                  default: () => item.rmk,
                  trigger: () => h('span', { class: 'tabler--question-circle' }),
                }),
                h('span', item.result),
              ]),
            }
  ...
  async function fetchData(startFromFirstPage = false) {
    ...
    try {
      ...
      const { data: fetchedData } = await onFetch({
        ...
        whereQuery: whereQuery.value, // TODO:
